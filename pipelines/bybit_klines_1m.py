"""
Bybit BTC/USDT 1-minute klines ingestion pipeline.

Pulls the last 100 one-minute candles from Bybit v5 public API,
normalizes them via the Bybit adapter, and upserts into DuckDB.
...
dag = DAG(
    dag_id="bybit_klines_1m",
    default_args=default_args,
    description="Ingest BTC/USDT 1m klines from Bybit into DuckDB",
    schedule_interval="*/5 * * * *",
    start_date=datetime(2025, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["phase-2", "crypto", "bybit"],
)


def extract(**kwargs) -> None:
    """Pull raw klines from Bybit v5 public REST API and push to XCom."""
    import sys
...
def load(**kwargs) -> None:
    ...
    try:
        ...
        con.execute(
            f"DELETE FROM curated.btc_ohlcv WHERE exchange = 'bybit' AND open_time IN ({placeholders})",
            open_times,
        )
        ...
        count = con.execute(
            "SELECT COUNT(*) FROM curated.btc_ohlcv WHERE exchange = 'bybit'"
        ).fetchone()[0]
        logger.info("Loaded %d rows. Bybit total: %d rows.", len(rows), count)
    finally:
        con.close()
